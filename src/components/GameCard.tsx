...
const GameCard: React.FC<GameCardProps> = ({ game, onPlay }) => {
  ...
  return (
    <div 
      className="group rounded-xl overflow-hidden relative flex flex-col card-hover-transform"
    ...
    >
      ...
      <div className="relative w-full overflow-hidden" style={{ height: `${IMAGE_HEIGHT}px` }}>
        ...
        <img
          ...
          className="w-full h-full object-cover card-image-transform"
          loading="lazy"
        />
      ...
      <div className="flex-1 p-4 flex flex-col" style={{ height: `${CONTENT_HEIGHT}px` }}>
        <div className="flex justify-between items-start mb-2">
          ...
          {/* Rating */}
          <div 
            className="flex items-center bg-gray-800 rounded-full px-2 py-1 transition-transform duration-200 flex-shrink-0 ml-1 group-hover:scale-105"
        ...
        <div className="flex flex-wrap gap-1 mb-3" style={{ minHeight: '26px', overflow: 'hidden' }}>
          {game.tags && game.tags.slice(0, 3).map((tag: string, index: number) => (
            <span 
              key={index} 
              className="text-xs px-2 py-0.5 bg-gray-800/80 text-gray-300 rounded-full transition-all duration-200 group-hover:opacity-100"
        ...
        <div className="flex justify-between items-center mt-auto gap-2">
          <button 
            onClick={() => onPlay && onPlay(game.id)}
            className="flex-1 py-1.5 text-sm rounded-lg flex items-center justify-center transition-all duration-300 whitespace-nowrap overflow-hidden text-white card-button-gradient"
            style={{ 
